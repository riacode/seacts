...
SWEEP_VARIANTS = {
    ...
    "best_structured_1step_lower_lr": {
        "rl_training": {
            **BEST_MODEL_OVERRIDES,
    ...
    "best_structured_1step_low_cost_bias": {
        "rl_training": {
            **BEST_MODEL_OVERRIDES,
...
def sweep_dqn() -> list[dict[str, float | str]]:
    ...
    summary_path = Path("/root/seacts/results/depmap_baselines/dqn_sweeps/sweep_summary.csv")
    for variant_name, overrides in SWEEP_VARIANTS.items():
        output_dir = Path("/root/seacts/results/depmap_baselines/dqn_sweeps") / variant_name
        ...
        results, output_path = run_dqn_training_pipeline(
            ...
            wandb_run_name=f"dqn-training-{variant_name}",
        )
        row = results.iloc[0].to_dict()
        row["variant"] = variant_name
        row["output_path"] = str(output_path)
