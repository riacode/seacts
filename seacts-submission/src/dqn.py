...
def load_structured_checkpoint_into_context(
    ...
    *,
    ...
    if encoder_weight_key in structured_state and encoder_weight_key in context_state:
        ...
        target_weight[:, :overlap] = source_weight[:, :overlap]  # partial dim transfer
    ...
    if freeze_shared_heads:
        ...
        if isinstance(context_network, ContextStructuredQNetwork):
            frozen_modules.append(context_network.select_head)  # not select-only variant
...
class DuelingMLPQNetwork(nn.Module):
    ...
    def forward(self, states: Any) -> Any:
        ...
        return value + advantage - advantage.mean(dim=1, keepdim=True)  # dueling centering
...
class StructuredQNetwork(nn.Module):
    ...
    def forward(self, states: Any) -> Any:
        ...
        global_context = encoded.mean(dim=1)  # pool across genes
        ...
        advantages = _torch_cat((query_q, select_q), dim=1)  # queries then selects
        if not self.dueling:
            return advantages
...
class ContextStructuredQNetwork(nn.Module):
    ...
    def forward(self, states: Any, context_indices: Any) -> Any:
        ...
        context_per_gene = context.unsqueeze(1).expand(-1, self.n_genes, -1)  # tile lineage
        done_expanded = done.unsqueeze(1).expand(-1, self.n_genes, -1)
...
class ContextSelectStructuredQNetwork(nn.Module):
    ...
    def forward(self, states: Any, context_indices: Any) -> Any:
        ...
        select_input = _torch_cat((head_input, context_per_gene), dim=2)  # lineage gates SELECT
...
class ContextFusionStructuredQNetwork(ContextStructuredQNetwork):
    ...
    def forward(self, states: Any, context_indices: Any) -> Any:
        ...
        context = self.lineage_embed(context_indices)
        modality_weights = torch.softmax(self.modality_weight_head(context), dim=-1)  # lineage prior

        obs_values = candidate_features[:, :, : self.n_modalities]
        obs_masks = candidate_features[:, :, self.n_modalities : 2 * self.n_modalities]  # query mask
        ...
        weighted_obs = obs_values * obs_masks * modality_weights.unsqueeze(1)  # weighted fusion
        ...
        select_q = torch.where(
            has_obs,
            select_q,
            torch.full_like(select_q, -1.0e9),  # mask empty SELECT
        )

        query_q = query_q + self.fusion_query_boost * modality_weights.unsqueeze(1)  # boost queries
...
def select_epsilon_greedy_action(
    ...
    if rng.random() < epsilon:
        ...
        if (
            ...
            return int(rng.choice(valid_select_indices.tolist()))  # biased SELECT explore
...
def optimize_dqn_batch(
    ...
    with torch.no_grad():
        ...
        discounts = torch.pow(torch.full_like(n_steps, gamma), n_steps)  # n-step gamma^k
