...
class EvidenceAcquisitionEnv:
    ...
    def _step_query(self, action: EvidenceAction) -> StepResult:
        ...
        if repeated:
            reward -= self.repeated_query_penalty  # repeat OK, extra cost
        else:
    ...
    def _step_select(self, action: EvidenceAction) -> StepResult:
        ...
        return StepResult(
            state=self.state,
            reward=-selected_dependency * self.selection_reward_scale,  # lower dep = better
            done=True,
            info={
...
def _modality_value(modality: pd.DataFrame, cell_line_id: str, gene: str) -> float:
    ...
    if numeric.empty:
        return float("nan")
    return float(numeric.mean())  # collapse duplicate rows
