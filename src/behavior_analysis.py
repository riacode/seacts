...
def join_cell_line_metadata(
    ...
    """Attach model metadata to per-episode DQN behavior rows."""
...
def summarize_failure_cases(episodes: pd.DataFrame, top_n: int = 20) -> pd.DataFrame:
    """Return episodes where the DQN missed or had large dependency regret."""
    frame = episodes.copy()
...
def summarize_success_cases(episodes: pd.DataFrame, top_n: int = 20) -> pd.DataFrame:
    """Return high-quality episodes, preferring hits with low query counts."""
    frame = episodes.copy()
    if "hit_at_k" in frame:
        frame = frame[frame["hit_at_k"] >= 1.0]
...
def summarize_query_efficiency(steps: pd.DataFrame) -> pd.DataFrame:
    """Summarize which true dependency ranks the DQN spends query actions on."""
...
def modality_usage_by_context(
    ...
    """Average per-modality DQN query counts by lineage or another context column."""
...
def write_behavior_analysis_tables(
    ...
    """Write reusable CSV summaries from DQN behavior logs."""
